...
public class Canica extends Object {
    ...
    public void tick() {
        ...
        if(clicked&&(state.getSelected()==null||state.getSelected()==this)){
            ...
            x=state.mouse.x-(state.mouse.hitbox.width/2)+all.getCamara().xOff;
            y=state.mouse.y-(state.mouse.hitbox.height/2)+all.getCamara().yOff;;
            assigned=false;
    ...
    public void render(Graphics g, boolean inHole, int id) {
        if(inHole){
            g.drawImage(all.sprites().canicas[id], (int)x-all.getCamara().xOff, (int)y-all.getCamara().yOff, null);
        }
    }
